...
pal1 <- colorNumeric(palette = "Blues", domain = sf_state_segregation$d_wb)
pal2 <- colorNumeric(palette = "Purple", domain = sf_state_segregation$i_b)
pal3 <- colorNumeric(palette = "Reds", domain = sf_state_segregation$e_b)
#pal1 <- colorBin("Purples", domain = sf_county_segregation$d_wb, bins = 5, na.color = "#808080")
...
sf_state_segregation |> 
  ...
  addPolygons(
    ...
    highlightOptions = highlightOptions(fillColor ="blue",
                                        bringToFront = TRUE),
    label=~paste0(state, ": ", round(d_wb, 2)),
  ...
  addLegend("bottomright", pal = pal1, values = ~sf_state_segregation$d_wb,
    ...
    opacity = 1, group="Dissiminarity Index") |> 
  addLegend("bottomright", pal = pal2, values = ~sf_state_segregation$i_b,
    title = "Isolation Index",
    opacity = 1, group="Isolation Index") |> 
  addLegend("bottomright", pal = pal3, values = ~sf_state_segregation$e_b,
    title = "Exposure Index",
    opacity = 1, group="Exposure Index") |>
  addLayersControl(baseGroups = c("Dissiminarity Index", "Isolation Index", "Exposure Index"),
                   position="topright",
...
```{r}
cleaned_demog <- readRDS("data_clean/ky_acs_cleaned_demog_20240717.rds")

cleaned_demog <- data_frame(county = cleaned_demog$county,
                                variable = cleaned_demog$variable,
                                estimate = cleaned_demog$estimate)

md_details <- cleaned_demog %>% 
  filter(county == params$NAME_place)
...
no_insurance <- md_details_wide %>% 
  select(county, noinsurance, totalpop) |> 
...
```{r include=FALSE}
race_eth_state <- get_acs(geography = "state",
                      state="KY",
                      output="tidy",
                      year=2019,
                      geometry = FALSE,
...
###### RACE & ETHNICITY
  ...
  mutate(
    county = rep("Kentucky State"),
    variable = ifelse(variable == "nhasian" | 
                             variable == "nhHIPI" | 
                             variable == "nhnative", "other", variable)) |> 
  group_by(county, variable) |> 
  summarise(estimate = sum(estimate)) |> 
  ungroup() |> 
  mutate(variable = case_when(
    variable == "hispanic" ~ "Hispanic", 
    variable == "nhb" ~ "Non-Hispanic Black",
    variable == "nhw" ~ "Non-Hispanic White",
    ...
    #label = paste0(c("Hispanic", "Non-Hispanic Black", "Non-Hispanic White", "Other"), " ", estimate)
    label = paste0(round((estimate /sum(estimate))*100,2), "%")
  ) 
...
p <- ggplot(data=race_eth, aes(x=fraction, y=level, fill = variable, label = label)) +
  geom_col(width = 0.5) +
  #scale_x_discrete(labels=c("Less than HS", "HS", "Some College", "Bachelor", "Graduate"))+
  labs(y="", x="") +
  ...
  geom_text(size = 2.5, fontface = "bold", position = position_fill(vjust = 0.7), 
            color = "black")+
    #scale_color_manual(values = c(rep("black", 2), rep("white", 2), "black")) +
  coord_flip() +
  ggtitle("Race and Ethinicity"  )
...
Gini 
![Gini index formula](images/gini.png)\
```{r}
state <- states(
  year = 2019,
  cb = TRUE,
...
incom_ineq_gini <- read_rds("data_clean/ky_acs_cleaned_demog_20240717.rds") |> 
  filter(variable == "incom_ineq_gini")

# Map Index of Disimilarity

state_of_interest = incom_ineq_gini %>%
  filter(county == params$NAME_place)

state_line <- state_of_interest %>% 
  st_sf()

# Getting GINI index data by zcta 
counties_zctas <- read_rds("ky_crosswalk_counties_zctas.rds")

gini_county_data <- read_rds("data_clean/ky_acs_cleaned_demog_20240717.rds") |> 
  filter(variable == "incom_ineq_gini") |> 
  filter(county == params$NAME_place)

gini_zcta_data <- get_acs(geography = "zcta",
                      state="KY",
                      #output="tidy",
                      year=2019,
                      geometry = TRUE,
  ...
  select(zcta = GEOID, totalpopE, incom_ineq_giniE, geometry) |> 
  right_join(counties_zctas, by = join_by(zcta)) |> 
  filter(!(is.na(incom_ineq_giniE))) |> 
  filter(county == params$NAME_place)
...
gini_zcta_data2 <- st_intersection(county, gini_zcta_data)

min <- min(gini_zcta_data2$incom_ineq_giniE) + 0.005
max <- max(gini_zcta_data2$incom_ineq_giniE) -  0.005
sep <- round(((max - min)/5), 2)

map_county <- ggplot() +
  geom_sf(data= gini_zcta_data2, aes(fill = incom_ineq_giniE),
          color = "white") +
    # geom_sf(data= ,
    #         color = "black") +
...
zoom_state <- st_bbox(ky_state) 
zoom_county <- st_bbox(gini_county_data) 
...
all_states_cropped <- st_crop(all_states, xmin = zoom_state[[1]]-4, #
                         ymin = zoom_state[[2]]-0.5,
                         xmax = zoom_state[[3]], 
                         ymax = zoom_state[[4]])
...
map_state <- ggplot() +
  ...
  geom_sf(data = incom_ineq_gini, aes(fill = estimate),
          color = "white") +
  geom_sf(data=gini_county_data,
          fill = NA,
          color = "black"
          ) +
  ...
  scale_fill_fermenter(
    ...
    direction = 1,
    palette = "YlGnBu") +
  labs(title = paste0(params$NAME_place, " County"),
       x = NULL,
       y = NULL) +
  theme_map() +
  theme(
  ...
  geom_rect(aes(xmin = zoom_county[[1]]-0.04, 
                xmax = zoom_county[[3]]+0.04, 
                ymin = zoom_county[[2]]-0.04, 
                ymax = zoom_county[[4]]+0.04), 
            color = "magenta", fill = NA,
            linewidth = 0.3) 



insert <- inset_element(map_county, 0, 0.1, 0.5, 0.6,
                           align_to = 'full'
                           )
